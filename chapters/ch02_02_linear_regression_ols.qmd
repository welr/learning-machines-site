...
# Three observations, so the column space of X is a plane in R^3 we can actually draw.
x3 = np.array([8., 12., 16.])
y3 = np.array([175., 255., 330.])
...
# A patch of the plane spanned by the two columns of X, centered on y_hat
u = np.linspace(-0.35, 0.35, 12)
U, V = np.meshgrid(u, u)
c0, c1 = X3[:, 0], X3[:, 1]
P = y_hat3[None, None, :] + U[..., None] * c0 + V[..., None] * c1

fig = plt.figure(figsize=(7.0, 5.4))
ax = fig.add_subplot(111, projection="3d")
ax.plot_surface(P[..., 0], P[..., 1], P[..., 2], color="#076FA1", alpha=0.18,
                edgecolor="#076FA1", linewidth=0.3)
ax.plot(*zip([0, 0, 0], y3), color="#E3120B", lw=2.2, label=r"$y$")
ax.plot(*zip([0, 0, 0], y_hat3), color="#31417A", lw=2.2, label=r"$\hat{y} = Hy$")
ax.plot(*zip(y_hat3, y3), color="#666666", lw=1.8, ls="--", label="residual")
...
# Empty 3-D box, then place the vectors by hand with trans3d()
lim <- c(0, 360)
pm <- persp(x = lim, y = lim, z = matrix(lim[1], 2, 2),
            xlim = lim, ylim = lim, zlim = lim,
            theta = 35, phi = 22, expand = 0.9, border = NA, ticktype = "detailed",
            nticks = 4, xlab = "obs 1", ylab = "obs 2", zlab = "obs 3",
            main = "Least squares is a projection onto the column space",
            col.axis = "#666666", cex.axis = 0.7)

c0 <- X3[, 1]; c1 <- X3[, 2]
corners <- rbind(y_hat3 - 0.35 * c0 - 0.35 * c1, y_hat3 + 0.35 * c0 - 0.35 * c1,
                 y_hat3 + 0.35 * c0 + 0.35 * c1, y_hat3 - 0.35 * c0 + 0.35 * c1)
polygon(trans3d(corners[, 1], corners[, 2], corners[, 3], pm),
        col = "#076FA133", border = "#076FA1", lwd = 1)

seg <- function(a, b, col, lwd = 2.2, lty = 1) {
  lines(trans3d(c(a[1], b[1]), c(a[2], b[2]), c(a[3], b[3]), pm),
        col = col, lwd = lwd, lty = lty)
}
O <- c(0, 0, 0)
seg(O, y3, "#E3120B")
seg(O, y_hat3, "#31417A")
seg(y_hat3, y3, "#666666", 1.8, 2)
legend("topleft", c("y", "y_hat = Hy", "residual"),
       col = c("#E3120B", "#31417A", "#666666"),
       lwd = c(2.2, 2.2, 1.8), lty = c(1, 1, 2), bty = "n", cex = 0.85)
```
