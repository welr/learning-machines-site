...
<figcaption><strong>The logistic curve.</strong> The fitted sigmoid maps hours studied to the probability of passing, bending between the failures (red, at zero) and the passes (blue, at one); the gray dashed line marks the 0.5 threshold, the boundary that turns a probability into a label. This is the result the code below reproduces.</figcaption>
</figure>
```
...
plot(thresholds, accs, type = "b", pch = 19, col = "#076FA1", lwd = 2,
     xlab = "Decision threshold", ylab = "Accuracy", xlim = c(0, 1))
