...
price in thousands. The prices were drawn once from a gently curved relationship,
$\text{price} = 100 + 2.5\,\text{area} - 0.005\,\text{area}^2$, plus Gaussian noise, so the
...
A parabola (degree 2) matches the quadratic truth. A degree-10 polynomial wiggles to chase
individual points. Note the training MSE: it keeps *falling* as the degree rises (796, 796,
446), so the worst-looking curve has the best training score. Training error alone cannot
tell you which model to trust; that is the job of held-out data, next.

```{=html}
<figure class="lm-figure">
<img src="../figures/ch02_01_polynomial_regression.png" alt="Fifteen house price-versus-area points with three polynomial fits: a degree-1 line and a degree-2 parabola nearly coincide through the data, while a degree-10 curve swings up and down to chase individual points.">
<figcaption><strong>One knob, three fits.</strong> The same fifteen houses fit by a degree-1 line (underfit), a degree-2 parabola that tracks the gentle curvature (good fit), and a degree-10 polynomial that wiggles to chase the noise (overfit). This is the result the code below reproduces.</figcaption>
</figure>
```
...
# price = 100 + 2.5*area - 0.005*area^2 + Normal(0, 25).
x = np.array([53.1, 58.7, 73.0, 73.4, 77.3, 81.9, 106.2, 139.8, 140.2,
              156.2, 159.8, 174.9, 179.9, 192.6, 195.5])
y = np.array([204.3, 206.4, 191.2, 280.3, 283.7, 233.0, 298.4, 333.2, 334.6,
              315.0, 356.1, 399.2, 451.9, 405.9, 400.7])
...
true = lambda a: 100 + 2.5 * a - 0.005 * a ** 2
xs = np.linspace(45, 200, 300)

fig, ax = plt.subplots(figsize=(8, 5))
ax.scatter(x, y, s=70, color="black", edgecolor="white", zorder=4, label="training data")
ax.plot(xs, true(xs), color="#666666", linestyle="--", linewidth=2, label="true relationship")
for degree, color, name in [(1, "#076FA1", "degree 1 (underfit)"),
...
x <- c(53.1, 58.7, 73.0, 73.4, 77.3, 81.9, 106.2, 139.8, 140.2,
       156.2, 159.8, 174.9, 179.9, 192.6, 195.5)
y <- c(204.3, 206.4, 191.2, 280.3, 283.7, 233.0, 298.4, 333.2, 334.6,
       315.0, 356.1, 399.2, 451.9, 405.9, 400.7)
...
true <- function(a) 100 + 2.5 * a - 0.005 * a^2
xs <- seq(45, 200, length.out = 300)

plot(x, y, pch = 21, bg = "black", col = "white", cex = 1.4,
     xlim = c(45, 200), ylim = c(100, 500),
     xlab = "Living area (m^2)", ylab = "Price (thousands)")
lines(xs, true(xs), col = "#666666", lty = 2, lwd = 2)
specs <- list(c(1, "#076FA1"), c(2, "#31417A"), c(10, "#E3120B"))
for (s in specs) {
...
legend("topleft", bty = "n",
       ...
       col = c("black", "#666666", "#076FA1", "#31417A", "#E3120B"),
...
sizes, this interpolating polynomial predicts *negative prices* over half the range, an
...
x_test = np.array([55.2, 56.8, 59.8, 61.2, 63.3, 64.7, 68.3, 71.1, 75.6, 77.7,
                   ...
                   195.4, 198.0])
y_test = np.array([209.8, 240.4, 253.7, 256.6, 257.0, 235.5, 231.8, 214.8, 287.9,
                   259.7, 256.7, 312.1, 263.2, 308.8, 300.9, 269.1, 296.3, 329.5,
                   332.0, 360.1, 341.9, 422.4, 319.2, 332.9, 350.1, 326.4, 391.2,
                   399.7, 377.9, 367.4, 344.1, 364.1, 409.5, 387.0, 382.8, 407.6,
                   424.8, 403.7, 405.5, 433.3])
...
x_test <- c(55.2, 56.8, 59.8, 61.2, 63.3, 64.7, 68.3, 71.1, 75.6, 77.7,
            ...
            170.3, 171.3, 174.3, 184.2, 186.4, 188.3, 190.9, 192.3, 194.8,
            195.4, 198.0)
y_test <- c(209.8, 240.4, 253.7, 256.6, 257.0, 235.5, 231.8, 214.8, 287.9,
            259.7, 256.7, 312.1, 263.2, 308.8, 300.9, 269.1, 296.3, 329.5,
            332.0, 360.1, 341.9, 422.4, 319.2, 332.9, 350.1, 326.4, 391.2,
            399.7, 377.9, 367.4, 344.1, 364.1, 409.5, 387.0, 382.8, 407.6,
            424.8, 403.7, 405.5, 433.3)
...
plot(degrees, train_err, type = "b", pch = 19, col = "#076FA1", log = "y", lwd = 2,
     ...
     xlab = "Polynomial degree", ylab = "Mean squared error (log scale)")
...
(intercept excluded). It grows from about 73 at degree 1 to several hundred by degree 10 and
to millions at the interpolating degree 14. This explosion is exactly what the next chapter's
ridge and LASSO penalties are built to prevent.
...
barplot(norms, names.arg = degrees, col = "#076FA1", border = "white", log = "y",
        xlab = "Polynomial degree", ylab = "Coefficient norm (log scale)")
