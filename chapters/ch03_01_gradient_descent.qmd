...
designs = {
    "raw features":  np.column_stack([np.ones(n), x1, x2]),
    "standardized":  np.column_stack([np.ones(n), standardize(x1), standardize(x2)]),
}
...
descend <- function(X, n_steps = 400) {
  ...
  for (k in seq_len(n_steps)) {
    ...
    theta <- theta - alpha * as.vector((-2 / n) * t(X) %*% (y - X %*% theta))
  }
