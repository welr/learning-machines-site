...
levels = np.logspace(np.log10(Loss.min() + 0.05), np.log10(Loss.max()), 18)
axL.contour(np.meshgrid(g0, g1)[0], np.meshgrid(g0, g1)[1], Loss,
            levels=levels, colors="#666666", linewidths=0.5, alpha=0.5)
axL.contourf(np.meshgrid(g0, g1)[0], np.meshgrid(g0, g1)[1], Loss,
             levels=levels, cmap="Blues", alpha=0.20)
...
::: {.explore}
[Try it]{.lbl}
The slider above already walks you across $2/\lambda_{\max} = 0.2592$: step from $0.25$ to
$0.26$ and a run that was merely zig-zagging turns into one whose loss grows by a fixed
factor every iteration. The boundary really is that sharp.

What the slider cannot show you is that the boundary *moves*. In the stability cell, multiply
`x` by 5 and rerun. Nothing about the problem has changed — the same line still fits the same
points — but the critical step size collapses from $0.2592$ to about $0.0125$, twenty times
smaller, because $\lambda_{\max}$ scales with the square of the feature. This is why features
are standardized before gradient descent is turned loose on them: the safe step size is a
property of the units you happened to measure in.
:::
