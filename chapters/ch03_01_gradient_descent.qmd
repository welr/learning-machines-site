...
ax.contourf(B0, B1, Loss, levels=levels, cmap="Blues_r", alpha=0.30)
    ...
    for b in np.linspace(-1.0, 2.75, 8):
        ...
        if nrm > 1e-6:
            ...
            ax.arrow(a, b, -step[0], -step[1], head_width=0.06,
                     head_length=0.05, fc="#31417A", ec="#31417A", alpha=0.65)

ax.scatter(*theta_ols, color="#E3120B", s=180, marker="*", zorder=5, label="OLS optimum")
...
contour(g0, g1, Loss, levels = levels, drawlabels = FALSE, col = "#666666",
...
points(theta_ols[1], theta_ols[2], col = "#E3120B", pch = 8, cex = 2, lwd = 2)
legend("topright", "OLS optimum", pch = 8, col = "#E3120B", bty = "n")
```
...
start = [2.0, -1.0]
rates = [(0.02, "#666666", "alpha = 0.02 (slow)"),
         (0.10, "#076FA1", "alpha = 0.10 (good)"),
         (0.25, "#E3120B", "alpha = 0.25 (oscillating)")]
...
axL.contourf(B0, B1, Loss, levels=levels, cmap="Blues_r", alpha=0.20)
...
axL.scatter(*theta_ols, color="#31417A", s=170, marker="*", zorder=6, label="optimum")
...
start <- c(2.0, -1.0)
rates <- list(list(0.02, "#666666", "alpha = 0.02 (slow)"),
              list(0.10, "#076FA1", "alpha = 0.10 (good)"),
              list(0.25, "#E3120B", "alpha = 0.25 (oscillating)"))
...
points(theta_ols[1], theta_ols[2], pch = 8, col = "#31417A", cex = 1.8, lwd = 2)
legend("topright", c(sapply(rates, `[[`, 3), "start", "optimum"),
       col = c(sapply(rates, `[[`, 2), "black", "#31417A"),
...
plot(NA, xlim = c(0, 40), ylim = range(unlist(loss_curves), opt_loss), log = "y",
     xlab = "iteration", ylab = "MSE loss", main = "Convergence")
for (k in seq_along(rates))
...
ax.plot(L_stable, "o-", color="#076FA1", ms=4, label="alpha = 0.25 (converges)")
ax.set_yscale("log")
...
lines(0:20, L_stable, type = "o", col = "#076FA1", pch = 19)
legend("bottomright", c("alpha = 0.30 (diverges)", "alpha = 0.25 (converges)"),
       col = c("#E3120B", "#076FA1"), lwd = 1.8, pch = 19, bty = "n")
```
...
plot(0:n_iter, L_batch, type = "l", col = "#31417A", lwd = 2, log = "y",
     ...
     main = "Batch vs stochastic gradient descent")
...
axL.contour(B0, B1, Loss, levels=levels, colors="#666666", linewidths=0.5, alpha=0.5)
axL.contourf(B0, B1, Loss, levels=levels, cmap="Blues_r", alpha=0.20)
axL.plot(P0[:, 0], P0[:, 1], "o-", color="#666666", ms=3, lw=1.4, label="plain GD")
axL.plot(P1[:, 0], P1[:, 1], "s-", color="#076FA1", ms=3, lw=1.4, label="momentum (gamma=0.8)")
axL.scatter(*start, color="black", s=70, zorder=5)
axL.scatter(*theta_ols, color="#E3120B", s=170, marker="*", zorder=6, label="optimum")
axL.set_xlabel(r"$\theta_0$")
axL.set_ylabel(r"$\theta_1$")
axL.set_title(f"Same step size alpha = {lr}")
axL.legend(loc="upper right", fontsize=8)

axR.plot([mse(*p) for p in P0], color="#666666", lw=2, label="plain GD")
axR.plot([mse(*p) for p in P1], color="#076FA1", lw=2, label="momentum (gamma=0.8)")
...
lines(P0[, 1], P0[, 2], type = "o", col = "#666666", pch = 19, cex = 0.5)
lines(P1[, 1], P1[, 2], type = "o", col = "#076FA1", pch = 15, cex = 0.5)
points(start[1], start[2], pch = 19, cex = 1.2)
points(theta_ols[1], theta_ols[2], pch = 8, col = "#E3120B", cex = 1.8, lwd = 2)
legend("topright", c("plain GD", "momentum (gamma=0.8)", "optimum"),
       col = c("#666666", "#076FA1", "#E3120B"), pch = c(19, 15, 8), bty = "n", cex = 0.8)

l0 <- apply(P0, 1, function(b) mse(b[1], b[2]))
l1 <- apply(P1, 1, function(b) mse(b[1], b[2]))
plot(0:n_iter, l0, type = "l", col = "#666666", lwd = 2, log = "y",
     ylim = range(l0, l1, opt_loss),
     xlab = "iteration", ylab = "MSE loss", main = "Momentum reaches the floor sooner")
lines(0:n_iter, l1, col = "#076FA1", lwd = 2)
abline(h = opt_loss, col = "#666666", lty = 2)
legend("topright", c("plain GD", "momentum (gamma=0.8)"),
       col = c("#666666", "#076FA1"), lwd = 2, bty = "n")
par(op)
