...
The reason is that $\lambda_{\max}$ grows roughly with the square of the feature scale. Only
roughly: squaring alone would predict a factor of 25, and the measured factor is 20.75,
because $\mathbf{X}^\top\mathbf{X}$ contains an intercept column that does not rescale with
$x$. This is why features are standardized before gradient descent is turned loose on them.
