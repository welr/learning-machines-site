...
<img src="../figures/ch01_01_baseline.png" alt="Scatter of twenty Frankfurt house sales, price against living area, with a dashed red horizontal line at the mean price and a blue least-squares line rising through the points. Thin gray vertical segments connect each point to the flat baseline.">
<figcaption><strong>The baseline, and what one feature buys.</strong> The dashed line predicts the same average for every house, ignoring living area; the gray segments are the error it leaves. Tilting that flat line to follow the data absorbs almost all of it. That is Chapter 2 in one picture.</figcaption>
</figure>
```

## The baseline and the error it leaves

Twenty sales: living area in square meters, price in thousands of euros. The baseline
...
# Same twenty sales as the first cell: every cell here stands on its own.
area = np.array([52, 82, 120, 95, 140, 160, 180, 200, 210, 225,
                 150, 110, 130, 170, 190, 205, 240, 60, 75, 100.])
price = np.array([164, 384, 465, 420, 520, 610, 700, 760, 820, 900,
                  560, 430, 500, 650, 720, 790, 980, 260, 330, 410.])
...
Both claims are easier to believe once you see them. The plot below draws itself when the
page opens.
...
price = np.array([164, 384, 465, 420, 520, 610, 700, 760, 820, 900,
                  560, 430, 500, 650, 720, 790, 980, 260, 330, 410.])
mse = ((price - price.mean()) ** 2).mean()          # the baseline to beat
...
price <- c(164, 384, 465, 420, 520, 610, 700, 760, 820, 900,
           560, 430, 500, 650, 720, 790, 980, 260, 330, 410)
mse <- mean((price - mean(price))^2)               # the baseline to beat
...
Over** to clear your attempt. They are Python only — the exercise machinery does not carry a
second tab — so R readers should treat them as worked problems.

The first asks what happens when a single observation is removed. Drop the largest sale, refit,
and compare how far each summary moves in units of its own standard error. Guess first.
...
def standard_errors(a, p):
    """SE of the mean, and SE of the least-squares slope."""
    m = len(a)
    _, slope = fit(a, p)
    intercept = p.mean() - slope * a.mean()
    resid = p - (intercept + slope * a)
    s2 = (resid ** 2).sum() / (m - 2)
    return p.std(ddof=1) / np.sqrt(m), np.sqrt(s2 / ((a - a.mean()) ** 2).sum())

mean_all, slope_all = fit(area, price)
se_mean, se_slope = standard_errors(area, price)

# Remove the single largest sale, then refit. Which index is the largest price?
keep = ______
mean_cut, slope_cut = fit(area[keep], price[keep])

# Report each shift in units of its own standard error: percentages of a price level and
# of a price per square meter are percentages of different things, and cannot be compared.
print(f"mean : {mean_all:7.2f} -> {mean_cut:7.2f}   moved {abs(mean_cut - mean_all)/se_mean:.2f} standard errors")
print(f"slope: {slope_all:7.3f} -> {slope_cut:7.3f}   moved {abs(slope_cut - slope_all)/se_slope:.2f} standard errors")
```
...
The slope moves $0.81$ standard errors; the mean moves $0.44$. Raw percentages would have told
you the opposite, but a percentage of a price level and a percentage of a price per square
meter are percentages of different things.

The largest sale is also the largest house, which puts it at the edge of the design: its
leverage is $0.197$ against an average of $0.10$, the highest of the twenty, and its residual
is the third largest. A point that is both far out in $x$ and off the line is one a slope
cannot ignore. Chapter 6 gives the observation its vocabulary — leverage, influence, and the
deletion diagnostics that measure them.
