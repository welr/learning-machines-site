...
The cell prints a percentage change for each, and the percentages invite a conclusion they
cannot support: the mean shifts about $3.8\%$ and the slope about $2.4\%$, but a percentage
of a price level and a percentage of a price-per-square-metre are not on the same scale, and
nothing follows from comparing them. Put both on the scale that does mean something — how far
each moved relative to its own standard error — and the ordering reverses: the slope moves
$0.81$ standard errors against the mean's $0.44$.

That is the answer the geometry predicts. The largest sale is also the largest house, which
makes it the highest-leverage point in this data set: its leverage is $0.197$ against an
average of $2/20 = 0.10$, the largest of the twenty. High-leverage points are exactly the ones
a fitted line cannot ignore, and this one sits well off the line as well — its residual is the
third largest of the twenty. A point with that combination moves a slope. Chapter 5 makes the
dependence precise; the vocabulary for it — leverage, influence, and the deletion diagnostics
that measure both — belongs to the regression chapter that follows this one.
