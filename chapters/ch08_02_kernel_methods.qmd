...
def make_rings(m=120):
    ...
    return X, np.r_[np.zeros(m), np.ones(m)].astype(int)
...
## R
```{webr}
make_rings <- function(m = 120) {
  theta <- seq(0, 2*pi, length.out = m + 1)[1:m]
...
legend("topright", c("outer", "inner"), col = c("#076FA1", "#E3120B"),
...
legend("topright", sprintf("threshold = %.1f", thresh), col = "#31417A",
...
for ax, g in zip(axes, [0.3, 2.0, 10.0, 50.0]):
    ...
    ax.scatter(X[y==0, 0], X[y==0, 1], c="#076FA1", s=14, label="outer")
    ax.scatter(X[y==1, 0], X[y==1, 1], c="#E3120B", s=14, label="inner")
    ax.set_title(f"$\\gamma$ = {g}   ({len(clf.support_vectors_)} SV)", fontsize=10)
    ax.set_aspect("equal"); ax.set_xticks([]); ax.set_yticks([])
axes[0].legend(loc="upper right", fontsize=8, framealpha=0.9)
plt.tight_layout(); plt.show()

print("Small gamma underfits (smooth); large gamma overfits (islands).")
print("Support vectors are fewest in between -- see the slider below.")
```
...
Four snapshots suggest the progression; the slider makes it continuous. Two things are worth
watching, and only one of them is the picture.

The first is that **training accuracy is 100% at every setting**, from the smoothest loop to
the most fractured archipelago. It is printed below precisely so you can watch it fail to
discriminate: the rings are separable, so fitting them perfectly says nothing about which
$\gamma$ generalizes. A diagnostic that returns the same verdict for every model is not a
diagnostic.

Held-out accuracy would not help either. These rings are two exact curves with no noise, so
fresh points drawn from them also score 1.000 at every $\gamma$. Where there is no noise,
nothing that measures agreement with labels can choose a bandwidth.

The second is the support-vector count, which does vary — but it counts how many points the
boundary needs in order to be described, which is the complexity of the fit rather than its
accuracy. It is not monotone in $\gamma$. It is high at *both* extremes and lowest in the
middle. At small $\gamma$ each point's influence is so broad that the margin is wide and
nearly every point ends up supporting it (all 240 at $\gamma = 0.1$). At large $\gamma$
influence is so local that the boundary shatters into islands, each needing its own points to
prop it up (188 at $\gamma = 100$). Between $\gamma = 2$ and $\gamma = 4$ the count bottoms
out in the twenties, reaching 22. Do not read that trough as the best model: a compact
boundary and a boundary that generalizes are related, but they are not the same thing.
...
viewof gam = {
  ...
  label.textContent = "bandwidth γ";
  const range = document.createElement("input");
  range.type = "range";
  // log scale: gamma = 10^v, from 0.1 to 100
  range.min = -1; range.max = 2; range.step = 0.1; range.value = 0.3;
  const out = document.createElement("output");
  const show = () => { out.textContent = Math.pow(10, Number(range.value)).toFixed(2); };
  show();
  range.addEventListener("input", show);
  range.addEventListener("change", () => {
    wrap.value = Math.pow(10, Number(range.value));
    wrap.dispatchEvent(new CustomEvent("input"));
  });
  const hint = document.createElement("p");
  hint.className = "hint";
  hint.textContent = "Logarithmic, from 0.1 to 100. Release the slider to refit.";
...
def rings(m=120):
    ...
    r_out = 1.00 + 0.08*np.cos(7*theta)
    r_in  = 0.40 + 0.08*np.sin(6*theta)
    P = np.vstack([np.column_stack([r_out*np.cos(theta), r_out*np.sin(theta)]),
                   np.column_stack([r_in *np.cos(theta), r_in *np.sin(theta)])])
    return P, np.r_[np.zeros(m), np.ones(m)].astype(int)
...
clf = SVC(kernel="rbf", gamma=g).fit(Xr, yr)
Z = clf.predict(grid).reshape(xx.shape)
n_sv = len(clf.support_vectors_)
acc = clf.score(Xr, yr)

fig, ax = plt.subplots(figsize=(5.4, 5.0))
ax.contourf(xx, yy, Z, alpha=0.25, cmap=ListedColormap(["#076FA1", "#E3120B"]))
ax.scatter(Xr[yr==0, 0], Xr[yr==0, 1], c="#076FA1", s=18,
           edgecolor="white", linewidths=0.4, label="outer")
ax.scatter(Xr[yr==1, 0], Xr[yr==1, 1], c="#E3120B", s=18,
           edgecolor="white", linewidths=0.4, label="inner")
ax.set_aspect("equal"); ax.set_xticks([]); ax.set_yticks([])
ax.set_title(rf"$\gamma$ = {g:.2f}   ({n_sv} of {len(Xr)} points are support vectors)",
             fontsize=11)
ax.legend(loc="upper right", fontsize=8, framealpha=0.9)
plt.tight_layout(); plt.show()

print(f"gamma = {g:.2f}  ->  {n_sv} of {len(Xr)} points are support vectors "
      f"({n_sv/len(Xr):.0%})")
print(f"training accuracy {acc:.1%} — unchanged across the whole sweep, and so useless "
      f"for choosing gamma")
```
...
plot(xs, ys, col = "#076FA1", pch = 19, xlab = "x", ylab = "y",
     main = "Normalize the kernel weights, then average: one attention head", bty = "l")
...
Then give the problem some noise, which is what it takes to make $\gamma$ selectable at all. In
the bandwidth cell, build a held-out set from the same curves with the radii perturbed, and
score each $\gamma$ on it:

```python
rng = np.random.default_rng()   # unseeded: rerun to see the winner move
th = np.linspace(0, 2*np.pi, 240, endpoint=False) + np.pi/240
ro = 1.00 + 0.08*np.cos(7*th) + rng.normal(0, 0.10, 240)
ri = 0.40 + 0.08*np.sin(6*th) + rng.normal(0, 0.10, 240)
Xte = np.vstack([np.column_stack([ro*np.cos(th), ro*np.sin(th)]),
                 np.column_stack([ri*np.cos(th), ri*np.sin(th)])])
yte = np.r_[np.zeros(240), np.ones(240)].astype(int)
print(f"gamma {g:6.2f}  held-out {clf.score(Xte, yte):.4f}")
```

Sweep the slider. The extremes now separate — $\gamma = 100$ collapses to about 0.94 — while
everything between sits within a point or two of perfect, and which setting wins moves with
the noise draw. That is the lesson, and it is a modest one: noise is what makes the question
answerable at all, not what makes it answerable precisely. Note only that the winner is not
reliably the support-vector minimum near $\gamma = 4$.
:::
