...
Only the [support vectors]{.term}, the points on or inside the margin, carry nonzero weight; the rest could be deleted without moving the boundary. Sklearn's `SVC` solves this directly. Base R ships no SVM (the production tools are `e1071` and `kernlab`), but the kernel idea needs no package. Form the [Gram matrix]{.term} $K_{ij} = k(x_i, x_j)$, then solve a kernel ridge regression $\alpha = (K + \lambda I)^{-1}\,y$ in that same Gram space. The weights $\alpha$ play the SVM's role, and a prediction is again a kernel-weighted sum over training points. Both routes turn a linear method nonlinear by swapping one dot product for a kernel.
...
cat(sprintf("rbf (g=2)  train accuracy = %5.1f%%\n", 100 * acc_rbf))
```
...
The RBF weights did something worth naming: they averaged labels by similarity. That is the Nadaraya-Watson kernel smoother of Chapter 8, and Chapter 11 shows it is also what one head of [attention]{.term} computes. A transformer scores a query against every key, passes the scores through a softmax to get weights, and returns the weighted average of the values. Trade "similarity to a key" for the Gaussian weight here, and "value" for the label, and attention is a kernel smoother whose similarity metric is learned rather than fixed. The kernel trick is no historical curiosity. It is the same idea the newest models run on.
