...
The second is the support-vector count, which is far more informative — and which is not
monotone in $\gamma$. It is high at *both* extremes and lowest in the middle. At small
$\gamma$ each point's influence is so broad that the margin is wide and nearly every point
ends up supporting it (all 240 at $\gamma = 0.1$). At large $\gamma$ influence is so local
that the boundary shatters into islands, each needing its own points to prop it up (188 at
$\gamma = 100$). Around $\gamma = 2$ the count bottoms out near 30: a boundary describable by
a handful of points is one that has found the structure rather than memorized the sample.
