...
Be careful how far you take that, though, because the honest version of this page's data is
harsher than it first appears. These rings are generated by two exact curves with no noise at
all, so *every* point of the population lies on one of them. Hold out fresh points from the
same two curves and the accuracy is 1.000 at every $\gamma$ as well. It is not that training
accuracy is the wrong yardstick here and something else is the right one; it is that this data
cannot separate these fits at all. Nothing that measures agreement with labels will choose
$\gamma$ for you on a problem with no noise in it.

The support-vector count does vary, and it is worth watching for a different reason: it
measures how many points the boundary needs in order to be described, which is a statement
about the complexity of the fit rather than its accuracy. It is not monotone in $\gamma$. It
is high at *both* extremes and lowest in the middle. At small $\gamma$ each point's influence
is so broad that the margin is wide and nearly every point ends up supporting it (all 240 at
$\gamma = 0.1$). At large $\gamma$ influence is so local that the boundary shatters into
islands, each needing its own points to prop it up (188 at $\gamma = 100$). Between
$\gamma = 2$ and $\gamma = 4$ the count bottoms out in the twenties, reaching 22.

Resist the temptation to read that trough as the best model. Add radial noise to held-out
points — the "Try it" box below does exactly this — and the best-generalizing bandwidth comes
out near $\gamma = 10$, with 55 support vectors, not at the count's minimum. A compact
boundary and a boundary that generalizes are related, but they are not the same thing, and on
this data only the first one is measurable.
...
def rings(n=120):
    ...
    P = np.vstack([np.column_stack([r_out*np.cos(theta), r_out*np.sin(theta)]),
                   np.column_stack([r_in *np.cos(theta), r_in *np.sin(theta)])])
    return P, np.r_[np.zeros(n), np.ones(n)].astype(int)
...
Sweep the slider and watch where the held-out score peaks. It is near $\gamma = 10$, at 0.998
— *not* at $\gamma \approx 4$ where the support-vector count is smallest (0.996), and far from
$\gamma = 100$, which collapses to 0.935. Without the noise every setting scored 1.000 and the
