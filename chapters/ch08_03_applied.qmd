...
make_moons <- function(m, noise = 0.30) {
  n_out <- m %/% 2; n_in <- m - n_out
  t_out <- seq(0, pi, length.out = n_out)
  t_in  <- seq(0, pi, length.out = n_in)
  x <- c(cos(t_out),      1 - cos(t_in))
  y <- c(sin(t_out), 0.5 - sin(t_in))
  X <- cbind(x, y) + matrix(rnorm(2 * m, sd = noise), ncol = 2)
  list(X = X, label = c(rep(0L, n_out), rep(1L, n_in)))
}

moons <- make_moons(300, noise = 0.30); X <- moons$X; lab <- moons$label
