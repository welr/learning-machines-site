...
for nneg in (200, 800, 3800, 19800):
    ...
    if nneg in drawn:
        ...
        a1.plot(fpr, tpr, color=colour, lw=2.5, label=label)
        a2.plot(rec, prec, color=colour, lw=2.5, label=label)
        a2.axhline(y.mean(), color=colour, ls=":", lw=1.5)
...
for (nneg in c(200, 800, 3800, 19800)) {
  ...
  if (nneg %in% c(200, 3800)) {
    colour <- if (nneg == 200) "#31417A" else "#E3120B"
    if (first) {
      plot(cv$rec, cv$prec, type = "l", col = colour, lwd = 2.5,
           xlim = c(0, 1), ylim = c(0, 1.05),
           xlab = "recall", ylab = "precision", bty = "l")
      grid(col = "#e6e3da", lty = 1, lwd = 0.6)
      first <- FALSE
    } else {
      lines(cv$rec, cv$prec, col = colour, lwd = 2.5)
    }
    abline(h = mean(y), col = colour, lty = 3, lwd = 1.5)
  }
}
