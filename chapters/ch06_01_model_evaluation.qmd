...
model can memorize. This page makes evaluation concrete. We fit a logistic classifier to a
roughly balanced, mildly nonlinear data set, then estimate its honest performance with
...
that trade-off with the [ROC curve]{.term} and its [AUC]{.term}, watch what becomes of
those summaries when positives turn rare, and finally ask whether the predicted
...
# Binary data with a mildly nonlinear boundary, 5% label noise.
m = 500
X = np.random.randn(m, 2) * 1.5
y = (X[:, 0] + 0.3 * X[:, 1]**2 > 0.5).astype(int)
flip = np.random.choice(m, size=int(0.05 * m), replace=False)
y[flip] = 1 - y[flip]
print(f"{m} samples, {y.mean():.0%} positive")
...
# Log-loss is the negative log-likelihood per observation: the very quantity the
# gradient descent above drove down. Accuracy scores a decision; this scores the
# probabilities the decision was made from.
def log_loss(i):
    p = np.clip(proba(X[i], theta), 1e-12, 1 - 1e-12)
    return -np.mean(y[i] * np.log(p) + (1 - y[i]) * np.log(1 - p))
print(f"train log-loss {log_loss(train):.3f}, test log-loss {log_loss(test):.3f}")

# 5-fold cross-validation: refit on each training split, score the held-out fold.
folds = np.array_split(np.random.permutation(m), 5)
scores = []
for k in range(5):
    te = folds[k]
...
flip <- sample(m, size = round(0.05 * m))
y[flip] <- 1 - y[flip]
...
# Single 80/20 split: optimistic on train, honest on the held-out test set.
idx <- sample(m)
test <- idx[1:100]; train <- idx[101:m]
...
# Log-loss is the negative log-likelihood per observation: exactly what glm
# maximized. Accuracy scores a decision; this scores the probabilities.
log_loss <- function(d) {
  p <- pmin(pmax(predict(fit, d, type = "response"), 1e-12), 1 - 1e-12)
  -mean(d$y * log(p) + (1 - d$y) * log(1 - p))
}
cat(sprintf("train log-loss %.3f, test log-loss %.3f\n",
            log_loss(df[train, ]), log_loss(df[test, ])))

# 5-fold cross-validation: refit on each training split, score the held-out fold.
folds <- split(sample(m), rep(1:5, length.out = m))
scores <- numeric(5)
for (k in 1:5) {
  te <- folds[[k]]; tr <- setdiff(1:m, te)
...
Notice the second pair of numbers. [Log-loss]{.term} is not one more score competing with
accuracy: it is the negative log-likelihood per observation, which is to say the exact
quantity the fit minimized. Every other metric on this page (accuracy, precision, recall,
the G-mean, AUC) summarizes a *decision rule* laid on top of the fitted probabilities, and
each depends on where you put the threshold. Log-loss scores the probabilities themselves,
before any threshold exists. That is why a model can win on accuracy and lose on log-loss:
it is being graded on two different things, only one of which it was trained to do.
...
The layout follows the book and decision theory: the classifier's predictions down the
rows, the actual classes across the columns, the positive class first on both axes.
Most software, scikit-learn included, prints the transpose with the negative class
first.
...
idx = np.random.permutation(m); test, train = idx[:100], idx[100:]
...
print(f"pred 1   |    {TP:5d}     {FP:5d}    (TP, FP)")
print(f"pred 0   |    {FN:5d}     {TN:5d}    (FN, TN)")
...
gmean = np.sqrt(recall * specificity)   # geometric mean of the two class-wise rates
...
set.seed(42)
m <- 500
x1 <- rnorm(m) * 1.5; x2 <- rnorm(m) * 1.5
y <- as.integer(x1 + 0.3 * x2^2 > 0.5)
flip <- sample(m, size = round(0.05 * m)); y[flip] <- 1 - y[flip]
df <- data.frame(x1 = x1, x2 = x2, y = y)

idx <- sample(m); test <- idx[1:100]; train <- idx[101:m]
fit <- glm(y ~ x1 + x2, family = binomial, data = df[train, ])
...
cat("            actual 1   actual 0\n")
cat(sprintf("pred 1   |    %5d     %5d    (TP, FP)\n", TP, FP))
cat(sprintf("pred 0   |    %5d     %5d    (FN, TN)\n", FN, TN))
...
specificity <- TN / (TN + FP)
f1 <- 2 * precision * recall / (precision + recall)
gmean <- sqrt(recall * specificity)   # geometric mean of the two class-wise rates
cat(sprintf("\naccuracy    %.3f\n", accuracy))
cat(sprintf("precision   %.3f\n", precision))
cat(sprintf("recall      %.3f\n", recall))
cat(sprintf("specificity %.3f\n", specificity))
cat(sprintf("F1          %.3f\n", f1))
cat(sprintf("G-mean      %.3f\n", gmean))
```
...
ax.plot(thr, prec, color="#31417A", lw=2.5, label="precision")
ax.plot(thr, rec, color="#2FC1D3", lw=2.5, label="recall")
...
plot(thr, prec, type = "l", col = "#31417A", lwd = 2.5, ylim = c(0, 1.05),
     xlab = "classification threshold", ylab = "score", bty = "l")
grid(col = "#e6e3da", lty = 1, lwd = 0.6)
lines(thr, rec, col = "#2FC1D3", lwd = 2.5)
abline(v = 0.5, col = "#666666", lty = 2)
legend("left", c("precision", "recall", "default threshold"),
       col = c("#31417A", "#2FC1D3", "#666666"),
...
ax.plot(fpr, tpr, color="#31417A", lw=3, label=f"ROC (AUC = {auc:.3f})")
...
# The AUC claim, checked directly: take every (positive, negative) pair and count
# how often the positive scores higher. This is the Mann-Whitney U statistic.
pos, neg = p[yt == 1], p[yt == 0]
wins = np.mean((pos[:, None] > neg[None, :]) + 0.5 * (pos[:, None] == neg[None, :]))
print(f"P(random positive scored above random negative) = {wins:.3f}")
```
...
plot(fpr, tpr, type = "l", col = "#31417A", lwd = 3, asp = 1,
     xlab = "false positive rate", ylab = "true positive rate", bty = "l")
grid(col = "#e6e3da", lty = 1, lwd = 0.6)
abline(0, 1, col = "#666666", lty = 2)
legend("bottomright", sprintf("ROC (AUC = %.3f)", auc),
       col = "#31417A", lwd = 3, bty = "n")
cat(sprintf("AUC = %.3f\n", auc))

# The AUC claim, checked directly: every (positive, negative) pair, counting how
# often the positive scores higher. This is the Mann-Whitney U statistic.
pos <- p[yt == 1]; neg <- p[yt == 0]
wins <- mean(outer(pos, neg, ">") + 0.5 * outer(pos, neg, "=="))
cat(sprintf("P(random positive scored above random negative) = %.3f\n", wins))
```
...
## When the positives are rare

AUC has a property that is easy to mistake for a virtue: it does not depend on how common
the positive class is. Both of its coordinates are rates *within* a class, so changing the
mix of classes leaves them alone. Precision is different. Its denominator counts every
case flagged positive, most of which are negatives when negatives dominate, so precision
falls as positives grow rarer even though the classifier has not changed at all.

The cleanest way to see this is to hold the classifier fixed and vary only the base rate.
Below, the same 200 positive cases keep the same scores throughout; all that changes is how
many negatives they are buried among, as if the same screening test were rolled out to an
ever-wider population. Alongside the ROC curve we trace the
[precision-recall curve]{.term} and its area, the [PR-AUC]{.term}.
...
def curves(p, y):
    """ROC and precision-recall coordinates from scores, by sorting once."""
    order = np.argsort(-p)
    ys = y[order]
    tp = np.cumsum(ys)
    fp = np.cumsum(1 - ys)
    ...
    return fpr, tpr, tpr, precision      # recall is the true positive rate

def area(x, y_):
    return np.trapezoid(np.concatenate([[0.0], y_]), np.concatenate([[0.0], x]))

SEP, NPOS = 1.6, 200
pos = np.random.normal(SEP, 1, NPOS)     # the same positive cases every time

print(f"{'screened':>9}{'prevalence':>12}{'ROC-AUC':>10}{'PR-AUC':>9}{'PR baseline':>13}")
fig, (a1, a2) = plt.subplots(1, 2, figsize=(11, 4.6))
drawn = {200: "#31417A", 3800: "#E3120B"}      # the two we also plot
for nneg in (200, 800, 3800, 19800):
    neg = np.random.normal(0, 1, nneg)
    p = 1 / (1 + np.exp(-np.concatenate([pos, neg])))
    y = np.concatenate([np.ones(NPOS, int), np.zeros(nneg, int)])
    fpr, tpr, rec, prec = curves(p, y)
    pr_auc = np.sum(np.diff(np.concatenate([[0.0], rec])) * prec)
    print(f"{NPOS+nneg:>9d}{y.mean():>11.0%}{area(fpr, tpr):>10.3f}"
          f"{pr_auc:>9.3f}{y.mean():>13.3f}")
    if nneg in drawn:
        color = drawn[nneg]
        label = f"{y.mean():.0%} positive"
        a1.plot(fpr, tpr, color=color, lw=2.5, label=label)
        a2.plot(rec, prec, color=color, lw=2.5, label=label)
        a2.axhline(y.mean(), color=color, ls=":", lw=1.5)

a1.plot([0, 1], [0, 1], color="#666666", ls="--", lw=1)
a1.set_xlabel("false positive rate"); a1.set_ylabel("true positive rate")
a1.set_title("ROC: barely moves")
a2.set_xlabel("recall"); a2.set_ylabel("precision")
a2.set_title("Precision-recall: collapses"); a2.set_ylim(0, 1.05)
for ax in (a1, a2):
    ax.legend(loc="best")
    for s in ["top", "right"]:
        ax.spines[s].set_visible(False)
...
roc_pr <- function(p, y) {
  ...
  tpr <- tp / sum(ys); fpr <- fp / sum(1 - ys)
  list(fpr = fpr, tpr = tpr, rec = tpr, prec = tp / seq_along(ys))
}
area <- function(x, y_) {
  x <- c(0, x); y_ <- c(0, y_)
  sum(diff(x) * (y_[-1] + y_[-length(y_)]) / 2)
}

SEP <- 1.6; NPOS <- 200
pos <- rnorm(NPOS, SEP, 1)               # the same positive cases every time

cat(sprintf("%9s %11s %9s %8s %12s\n",
            "screened", "prevalence", "ROC-AUC", "PR-AUC", "PR baseline"))
first <- TRUE
for (nneg in c(200, 800, 3800, 19800)) {
  neg <- rnorm(nneg, 0, 1)
  p <- 1 / (1 + exp(-c(pos, neg)))
  y <- c(rep(1L, NPOS), rep(0L, nneg))
  cv <- roc_pr(p, y)
  pr_auc <- sum(diff(c(0, cv$rec)) * cv$prec)
  cat(sprintf("%9d %10.0f%% %9.3f %8.3f %12.3f\n",
              NPOS + nneg, 100 * mean(y), area(cv$fpr, cv$tpr), pr_auc, mean(y)))
  if (nneg %in% c(200, 3800)) {
    color <- if (nneg == 200) "#31417A" else "#E3120B"
    if (first) {
      plot(cv$rec, cv$prec, type = "l", col = color, lwd = 2.5,
           xlim = c(0, 1), ylim = c(0, 1.05),
           xlab = "recall", ylab = "precision", bty = "l")
      grid(col = "#e6e3da", lty = 1, lwd = 0.6)
      first <- FALSE
    } else {
      lines(cv$rec, cv$prec, col = color, lwd = 2.5)
    }
    abline(h = mean(y), col = color, lty = 3, lwd = 1.5)
  }
}
legend("topright", c("50% positive", "5% positive"),
       col = c("#31417A", "#E3120B"), lwd = 2.5, bty = "n")
```

:::

Read the ROC-AUC column downward and it barely stirs; read the PR-AUC column and it falls
away. Nothing about the classifier changed between those rows: the same cases, the same
scores, the same ranking. Only the population being screened changed. AUC's independence
from prevalence is exactly what makes it the right summary when the task is to *rank*
candidates, and exactly what makes it a poor guide to what a deployed screening test will
feel like, where one in a hundred is positive and most alarms are false. The dotted lines
mark each setting's PR baseline, the precision a coin flip would achieve, which is just the
prevalence itself; a PR curve is only impressive relative to that line.

This is a base-rate argument, the same one that makes a positive result on a rare-disease
test less alarming than it first appears. The metric did not lie in either case; it answered
...
plot(mean_pred, frac_pos, type = "b", col = "#31417A", lwd = 2.5, pch = 19,
     xlim = c(0, 1), ylim = c(0, 1), asp = 1,
     xlab = "mean predicted probability", ylab = "fraction positive", bty = "l")
grid(col = "#e6e3da", lty = 1, lwd = 0.6)
abline(0, 1, col = "#666666", lty = 2)
legend("topleft", c("logistic regression", "perfectly calibrated"),
       col = c("#31417A", "#666666"), lwd = c(2.5, 1),
...
No single number settles whether a model is good. Accuracy can flatter a classifier facing
lopsided classes; the confusion matrix names the errors; the threshold sets the
precision-recall balance for the task at hand; AUC measures ranking independent of any
cutoff, which is a virtue when you are ranking and a blind spot when positives are rare;
PR-AUC restores the base rate to the picture; and calibration asks whether the probabilities
can be taken at face value. Log-loss sits apart from all of them, scoring the probabilities
the model was actually fit to produce. Choose the metric that matches the question you are
asking, and the cost you actually face.
