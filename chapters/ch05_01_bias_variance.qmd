...
<figcaption><strong>The bias–variance trade-off.</strong> As complexity grows, bias falls while variance rises; their sum, the total error, traces a U and bottoms out at degree three. The fall to three is a cliff rather than a slope: $\sin(2\pi x)$ is odd about $x = 1/2$, so an even degree adds variance without buying any reach, and bias$^2$ edges back up at every even step. This is the result the code below reproduces.</figcaption>
</figure>
```
...
Every prediction below is clipped to $[-10, 10]$ before the decomposition runs, so these
