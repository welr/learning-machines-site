...
<img src="../figures/ch05_01_bias_variance.png" alt="Bias-squared, variance, and total error versus polynomial degree; total error traces a U and bottoms out at a moderate degree.">
<figcaption><strong>The bias–variance trade-off.</strong> As complexity grows, bias falls while variance rises; their sum, the total error, traces a U and bottoms out at a moderate degree. This is the result the code below reproduces.</figcaption>
</figure>
```
...
The noise level $\sigma$ is fixed at 0.3 above. Move it, and the result is worth predicting
before you look: the dashed irreducible line rises as $\sigma^2$, and the whole U lifts with
it. What does *not* move is the winner. Degree three keeps taking the honors across almost the
whole range; only at the quietest setting, $\sigma = 0.05$, is there enough signal to justify
a more flexible degree five.

That stability is the point. The noise level decides how well any model can do — the floor is
$\sigma^2$ and nothing gets under it — but it barely touches which model does best, because
that is a question about the shape of $\sin(2\pi x)$, not about the noise laid over it.
...
::: {.explore}
[Try it]{.lbl}
Take the noise slider down to $0.10$. The irreducible floor drops to $0.01$, as
$\sigma^2$ must — but watch what does *not* happen: the winner is still degree three. Keep
going to $0.05$ and only then does degree five take over. The lesson is worth more than the
one you might have expected: cleaner data mostly buys you a lower floor, not a licence to
fit a wigglier curve.

For the sharper version of the same question, change `n=20` to `n=100` in the cell above and
rerun. *That* moves the optimum: degree five wins, and the variance at degree twelve falls
from about $9.8$ to $0.05$. More data, not quieter data, is what pays for complexity — noise
sets the floor, but sample size decides how much flexibility you can afford above it.
:::
