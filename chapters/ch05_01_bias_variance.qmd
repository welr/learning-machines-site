...
<img src="../figures/ch05_01_bias_variance.png" alt="Bias-squared, variance and total error against polynomial degree on a logarithmic scale. Variance climbs steadily, bias-squared falls in a zigzag, and the total error reaches its lowest point at degree three before rising by two orders of magnitude.">
<figcaption><strong>The bias–variance trade-off.</strong> Variance climbs steadily with degree while bias falls, and their sum bottoms out at degree three before rising by two orders of magnitude — the vertical scale is logarithmic, because the right-hand tail will not otherwise share an axis with the rest. Bias falls in a zigzag rather than smoothly: $\sin(2\pi x)$ is odd about $x = 1/2$, so an even degree adds variance without buying any reach. This is the result the code below reproduces, and `tools/make_ch05_01_figure.py` runs the same computation.</figcaption>
</figure>
```
...
twelve falls from about $9.8$ to $0.05$; without the clip the fall would be steeper still.
:::
