...
<img src="../figures/ch05_01_bias_variance.png" alt="Bias-squared, variance, and total error versus polynomial degree; variance climbs steadily while bias falls in steps, and the total error reaches its lowest point at a moderate degree before climbing.">
<figcaption><strong>The bias–variance trade-off.</strong> As complexity grows, variance climbs steadily while bias falls; their sum bottoms out at a moderate degree and rises after it. Bias falls in steps rather than smoothly, because $\sin(2\pi x)$ is odd about $x = 1/2$ and an even degree buys nothing over the odd degree below it — so the total error dips at each odd degree and edges back up at each even one. This is the result the code below reproduces.</figcaption>
</figure>
```
...
def fit_many(degree, n_datasets=60, n=20):
    ...
    for s in range(n_datasets):
        ...
        # Clipped: the decomposition below is of this clipped predictor, not the raw
        # polynomial. At high degree the clip binds and shrinks the reported variance.
...
One detail in the code below is not cosmetic and should be stated plainly, because it
changes what the word "variance" refers to on this page. Every prediction is clipped to
$[-10, 10]$ before the decomposition is computed, so what is being decomposed is a
*clipped* predictor, not the raw polynomial. At low degree the clip never binds and the
distinction is empty. At high degree it binds hard, and it is doing real work: at degree
twelve with twenty points, the clipped variance is about $9.8$, while the same quantity
without the clip is roughly $2.4 \times 10^5$. A handful of catastrophic extrapolations
dominate that number, which is why the clip is there — an unclipped axis would be
unreadable — but the honest reading of the plots below is that they show the trade-off for
a predictor that has been told not to leave the room. The shape of the story is unchanged;
the magnitude of the right-hand tail is drastically understated.
...
plot(degrees, rows[, 4], type = "o", pch = 17, col = "#E3120B", lwd = 2,
     ...
     xlab = "polynomial degree (model complexity)", ylab = "error", bty = "l")
grid(col = "#e6e3da", lty = 1, lwd = 0.6)
...
The noise level $\sigma$ is fixed at 0.3 above. Move it and watch two things separately:
...
# Fewer datasets than the table above (40 rather than 100) so the redraw stays quick.
def decompose_at(degree, n_datasets=40, n=20, noise=0.3):
    grid = np.linspace(0, 1, 100)
    truth = truth_f(grid)
    preds = []
    for s in range(n_datasets):
        ...
        yt = truth_f(xt) + np.random.normal(0, noise, n)
        coef = np.polyfit(xt, yt, degree)
        # Clipped: the decomposition below is of this clipped predictor, not the raw
        # polynomial. At high degree the clip binds and shrinks the reported variance.
        preds.append(np.clip(np.polyval(coef, grid), -10, 10))
...
::: {.explore}
[Try it]{.lbl}
Take the noise slider down to $0.10$. The irreducible floor drops to $0.01$, as $\sigma^2$
...
than learn from it. At forty datasets the two degrees are within Monte Carlo error of each
...
For the version that does move the optimum, change `n=20` to `n=100` in the cell above and
rerun: degree five wins outright. Both effects reduce variance, but sample size reduces it
far faster than noise does at the settings anyone would call realistic. (The variance the
plot reports at degree twelve falls from about $9.8$ to $0.05$ — but read that against the
clipping note above, since at $n=20$ the clip is binding hard and at $n=100$ it is not, so
the drop in the *unclipped* variance is far larger still.)
:::
