...
def decompose(degree, n_datasets=100, m=20, noise=0.3):
    grid = np.linspace(0, 1, 100)
    truth = true_f(grid)
    preds = []
    for s in range(n_datasets):
...
decompose <- function(degree, n_datasets = 100, m = 20, noise = 0.3) {
...
twelve falls from about $9.9$ to $0.07$; without the clip the fall would be steeper still.
:::
