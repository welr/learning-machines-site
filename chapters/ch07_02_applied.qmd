---
title: "Applied — Ridge, LASSO, and the Regularization Path"
engine: markdown
---
...
<figcaption><strong>The LASSO path.</strong> As the penalty &lambda; grows, each coefficient is driven to <em>exactly</em> zero one at a time; the eight informative features (blue) survive deepest into the penalty while the spurious ones (gray) hug zero and are the first driven out &mdash; LASSO selects variables by driving coefficients exactly to zero. (In this simulation the features are uncorrelated; with correlated predictors, survival depth is not a ranking of importance.) This is the result the code below reproduces.</figcaption>
</figure>
```
...
```{webr}
set.seed(0)
m <- 200; n <- 30; k <- 8                 # 200 rows, 30 features, 8 with signal

X <- scale(matrix(rnorm(m * n), m, n))    # standardize: each column mean 0, sd 1
true_idx <- sort(sample(n, k))            # which features carry signal
theta_true <- numeric(n)
theta_true[true_idx] <- runif(k, 10, 100)  # their coefficients
y <- as.vector(X %*% theta_true) + rnorm(m, 0, 10)
y <- y - mean(y)                          # center the target, so no intercept is needed

cat(n, "features;", sum(theta_true != 0), "of them actually matter\n")
...
set.seed(0)
m <- 200; n <- 30; k <- 8
X <- scale(matrix(rnorm(m * n), m, n))
true_idx <- sort(sample(n, k))
theta_true <- numeric(n); theta_true[true_idx] <- runif(k, 10, 100)
y <- as.vector(X %*% theta_true) + rnorm(m, 0, 10)
y <- y - mean(y)
...
lasso_cd <- function(X, y, lambda, n_iter = 200, tol = 1e-7) {
  ...
  for (it in seq_len(n_iter)) {
    ...
    for (j in seq_len(n)) {
      ...
      rho <- sum(X[, j] * r)
      theta[j] <- sign(rho) * max(abs(rho) - m * lambda, 0) / cj[j]  # soft-threshold
...
lam_max  <- max(abs(crossprod(X, y))) / m
...
ridge_grid <- 10^seq(-1, 3.5, length.out = 40) * m
...
# ridge: spurious gray behind, informative blue on top
matplot(log10(ridge_grid / m), ridge_path, type = "n", bty = "l",
...
matlines(log10(ridge_grid / m), ridge_path[, spur], lty = 1, lwd = 0.9, col = SPUR)
matlines(log10(ridge_grid / m), ridge_path[, info], lty = 1, lwd = 1.8, col = INFO)
...
lasso_cd <- function(X, y, lambda, n_iter = 200, tol = 1e-7) {
  m <- nrow(X); n <- ncol(X)
  theta <- numeric(n); cj <- colSums(X^2); r <- y - X %*% theta
  for (it in seq_len(n_iter)) {
    theta_old <- theta
    for (j in seq_len(n)) {
      r <- r + X[, j] * theta[j]
      rho <- sum(X[, j] * r)
      theta[j] <- sign(rho) * max(abs(rho) - m * lambda, 0) / cj[j]
      r <- r - X[, j] * theta[j]
    }
    if (max(abs(theta - theta_old)) < tol) break
  }
...
K <- 5
folds <- sample(rep(1:K, length.out = m))
