...
<figcaption><strong>The LASSO path.</strong> As the penalty &lambda; grows, each coefficient is driven to <em>exactly</em> zero one at a time; the eight informative features (blue) survive deepest into the penalty while the spurious ones (gray) never leave the zero line, which is how the LASSO selects variables. This is the result the code below reproduces.</figcaption>
</figure>
```
...
a1.plot(np.log10(alphas), ridge_coefs, color="#076FA1", lw=0.8, alpha=0.7)
a1.set_title("ridge"); a1.set_xlabel(r"$\log_{10}$ penalty"); a1.set_ylabel("coefficient")
a2.plot(np.log10(lasso_alphas), lasso_coefs.T, color="#31417A", lw=0.8, alpha=0.7)
...
par(mfrow = c(1, 2))
matplot(log10(ridge_grid / n), ridge_path, type = "l", lty = 1, lwd = 1,
        col = "#076FA1", main = "ridge",
        xlab = "log10 penalty", ylab = "coefficient")
matplot(log10(lam_grid), lasso_path, type = "l", lty = 1, lwd = 1,
        col = "#31417A", main = "LASSO",
        xlab = "log10 penalty", ylab = "coefficient")
